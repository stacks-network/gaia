...
`blockstack.js` APIs
documented [here](https://github.com/blockstack/blockstack.js/tree/master/src/storage).
...
for S3 and Azure Blob Storage, however, the driver model allows for other backend support
...
This performs a write to the gaia hub at `${path}`. The path must
begin with a 
